...
export default function Profile() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      // Le ?t= force la récupération des dernières commandes
  ...
  const downloadFile = async (fileId: string) => {
    try {
      ...
      if (response.data.downloadUrl) {
        // ✅ MÉTHODE RADICALE : On redirige directement vers l'URL
        // Cela évite les blocages de "fenêtres surgissantes" sur ton PC
